...
function getElementValue(element) {
  if (!element) return "";
  if (Object.keys(element.dataset || {}).some((key) => key.endsWith("Status")))
    return "";
...
function getToolState() {
  ...
  return [
    ...root.querySelectorAll(
      "input, textarea, select, output, pre, code, [contenteditable='true'], [data-json-output]",
    ),
  ]
...
function getCopySource(actionName) {
  ...
  if (actionName.includes("binary"))
    return getFirstValue(["[data-binary-output]"]);
  if (actionName.includes("hex")) return getFirstValue(["[data-hex-output]"]);
  if (actionName.includes("json"))
    return getFirstValue(["[data-json-output]", "[data-json-input]"]);
  if (actionName.includes("jm")) return getFirstValue(["[data-jm-output]"]);
  if (actionName.includes("md") && actionName.includes("html"))
    return getFirstValue(["[data-md-preview]"]);
  if (actionName.includes("md")) return getFirstValue(["[data-md-input]"]);
  if (actionName.includes("regex"))
    return getFirstValue(["[data-regex-output]", "[data-regex-result]"]);
  if (actionName.includes("timeduration"))
    return getFirstValue([
      "[data-time-duration-output]",
      "[data-time-duration-result]",
    ]);
  if (actionName.includes("timetimestamp"))
    return getFirstValue([
      "[data-time-timestamp-output]",
      "[data-time-timestamp-result]",
    ]);
...
function shouldCountAction(element, beforeState, beforeSuccessCount) {
  ...
  const afterSuccessCount = document.querySelectorAll(
    "[data-type='success']",
  ).length;
  const isCopyLike = ["copy", "export", "download"].some((word) =>
    actionName.includes(word),
  );
  const isClearLike = ["clear", "reset"].some((word) =>
    actionName.includes(word),
  );
  ...
  return (
    afterSuccessCount > beforeSuccessCount ||
    (beforeState !== afterState &&
      (hasValue(beforeState) || hasValue(afterState)))
  );
}
...
function isLocalLimitReached(toolId) {
  try {
    ...
    const clicks = (Array.isArray(store[toolId]) ? store[toolId] : []).filter(
...
function renderMostUsedTools(containers, tools = []) {
  containers.forEach((container) => {
    if (!tools.length) {
      container.innerHTML = '<span class="nav-empty">/</span>';
      return;
    }
    container.innerHTML = tools
      .map((tool) => {
        ...
        const label =
          Number(tool.submitCount || 0) > 0
            ? `${name} - ${Number(tool.submitCount || 0)} submit`
            : name;
...
export async function recordToolSubmit(toolId = getCurrentToolId()) {
  ...
  try {
    const response = await fetch(
      `/api/tools/${encodeURIComponent(toolId)}/submit`,
      {
        method: "POST",
        headers: {
...
export function setupToolSubmitTracking() {
  ...
  document.addEventListener(
    ...
    (event) => {
      ...
      clickSnapshot = {
        ...
        beforeSuccessCount: document.querySelectorAll("[data-type='success']")
          .length,
      };
    },
    true,
  ...
  document.addEventListener("click", (event) => {
    const actionElement = getTrackedActionElement(event, toolId);
    if (!actionElement || clickSnapshot?.actionElement !== actionElement)
      return;
    const { beforeState, beforeSuccessCount } = clickSnapshot;
    clickSnapshot = null;
    window.setTimeout(() => {
      if (shouldCountAction(actionElement, beforeState, beforeSuccessCount))
